...
export const ExpirationDataTables: React.FC<ExpirationDataTablesProps> = ({ data, onRowClick }) => {
  ...
  // Table 1: Expired Memberships
  const expiredData = filteredAndSortedData.filter(item => item.status === 'Expired');

  // Table 2: Expiring This Month
  const expiringData = filteredAndSortedData.filter(item => item.status === 'Expiring This Month');
  ...
  const summaryData = data.reduce((acc, item) => {
    ...
    if (!acc[key]) {
      acc[key] = { name: key, total: 0, active: 0, expired: 0, expiring: 0 };
    }
    acc[key].total++;
    if (item.status === 'Active') acc[key].active++;
    if (item.status === 'Expired') acc[key].expired++;
    if (item.status === 'Expiring This Month') acc[key].expiring++;
    return acc;
  }, {} as Record<string, { name: string; total: number; active: number; expired: number; expiring: number }>);
  ...
  const getStatusBadge = (status: string) => (
    <Badge
      ...
      className={cn(
        ...
        status === 'Expiring This Month' ? "border-yellow-200 bg-yellow-50 text-yellow-700" :
        ...
        <Card className="bg-white/70 backdrop-blur-sm border-slate-200/50">
          <CardHeader>
            <CardTitle className="text-lg font-semibold bg-gradient-to-r from-red-600 to-red-500 bg-clip-text text-transparent">
              Expired Memberships ({formatNumber(expiredData.length)})
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className="overflow-x-auto max-h-96">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {expiredData.slice(0, 50).map((item, index) => (
                    <tr 
                      key={index} 
        ...
        <Card className="bg-white/70 backdrop-blur-sm border-slate-200/50">
          <CardHeader>
            <CardTitle className="text-lg font-semibold bg-gradient-to-r from-yellow-600 to-yellow-500 bg-clip-text text-transparent">
              Expiring This Month ({formatNumber(expiringData.length)})
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className="overflow-x-auto max-h-96">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {expiringData.slice(0, 50).map((item, index) => (
                    <tr 
                      key={index} 
                      className="border-b border-slate-100 hover:bg-slate-50/50 cursor-pointer"
                      onClick={() => onRowClick?.(item)}
              ...
              <table className="w-full text-sm">
                <thead className="sticky top-0 bg-white/90">
                  <tr className="border-b border-slate-200">
                    ...
                    <th className="text-center py-2 px-3 font-medium text-yellow-600">Expiring</th>
                    <th className="text-center py-2 px-3 font-medium text-red-600">Expired</th>
                  </tr>
                </thead>
                <tbody>
                  {summaryArray.map((item, index) => (
                    <tr key={index} className="border-b border-slate-100 hover:bg-slate-50/50">
                      ...
                      <td className="py-2 px-3 text-center text-yellow-600">{formatNumber(item.expiring)}</td>
                      <td className="py-2 px-3 text-center text-red-600">{formatNumber(item.expired)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
