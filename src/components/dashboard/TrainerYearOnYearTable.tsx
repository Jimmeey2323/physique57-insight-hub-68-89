...
export const TrainerYearOnYearTable: React.FC<TrainerYearOnYearTableProps> = ({ 
  ...
  const yearOnYearData = useMemo(() => {
      ...
      if (!isNaN(yearNum) && (yearNum === currentYear || yearNum === previousYear)) {
        if (!acc[trainerKey]) {
          acc[trainerKey] = {
            ...
            currentYear: { sessions: 0, revenue: 0, customers: 0, monthsActive: 0 },
            previousYear: { sessions: 0, revenue: 0, customers: 0, monthsActive: 0 }
          };
        }
    ...
    return Object.values(trainerStats)
      ...
      .filter(trainer => trainer.currentSessions > 0 || trainer.previousSessions > 0)
